...
ON PODCAT2.id = TOV.id_podcat2 AND PODCAT2.id_podcat = PODCAT1.id ".$where."
 ORDER BY CAT.name_rus";
	...
	if ($result = $mysqli->query($query)) {
		if ($result->num_rows == 0) echo '<span id="no-products">Отсутствует товар по выбранным значениям</span>';
...
//			print_r($_SESSION);
			if($_REQUEST['logout'] == '1' || (!isset($_SESSION['desi']) || $_SESSION['desi']==0)) $cost = 'Уточните цену у менеджера';
//			print "@@<pre>";
//                        print_r($_SESSION);
//                        print "</pre>!!";
